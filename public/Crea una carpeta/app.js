/* app.js - complet i autònom
   ...
   - ENVÍO DE EMAIL AL CLIENTE CON FACTURA (2 cuentas EmailJS)
*/
...
/* ---------------------------
   INICIALITZACIÓ EMAILJS - DOS COMPTES
   --------------------------- */
// Compte principal (administració) - miuartbase@gmail.com
emailjs.init('USER_ID_CUENTA_1');

// Compte clients - miuartclientes@gmail.com  
const emailjsClientes = emailjs.init('USER_ID_CUENTA_2');
...
if (contactForm) {
  contactForm.addEventListener('submit', function(event) {
    ...
    const serviceID = 'xxxx'; // Pot ser el mateix servei
    const templateID = 'xxxx'; // Template DIFERENT per al contacte
...
/* ---------------------------
   GUARDAR COMANDA I ENVIAR EMAIL (EmailJS) - DOS COMPTES
   --------------------------- */
async function guardarComanda(datosFormulario) {
  try {
    ...
    const listaProductosHTML = comandaData.productos.map(producto => 
    ...
    const listaProductosClienteHTML = comandaData.productos.map(producto => 
      `<tr>
        <td style="padding: 10px; border-bottom: 1px solid #eee;">${producto.nombre} (${producto.variante})</td>
        <td style="padding: 10px; border-bottom: 1px solid #eee; text-align: center;">${producto.cantidad}</td>
        <td style="padding: 10px; border-bottom: 1px solid #eee; text-align: right;">${formatPrice(producto.precio)}</td>
        <td style="padding: 10px; border-bottom: 1px solid #eee; text-align: right;">${formatPrice(producto.precio * producto.cantidad)}</td>
      </tr>`
    ).join('');

    // 4. ENVIAR EMAIL AL ADMINISTRADOR (Compte 1 - miuartbase@gmail.com)
...
${datosFormulario.pais}
      `.trim(),
      llista_productes: listaProductosHTML,
    ...
    // ⚠️ REEMPLAÇA AMB LES TEVES CLAUS REALS DEL COMPTE 1 ⚠️
    const serviceIDAdmin = 'SERVICE_ID_CUENTA_1';
    const templateIDAdmin = 'TEMPLATE_ID_ADMIN';

    console.log('📧 Enviant email de comanda a la tienda...');
    await emailjs.send(serviceIDAdmin, templateIDAdmin, templateParamsAdmin);
    console.log('✅ Email enviat correctament a miuartbase@gmail.com');

    // 5. ENVIAR EMAIL DE CONFIRMACIÓN AL CLIENTE (Compte 2 - miuartclientes@gmail.com)
    const templateParamsCliente = {
      client_nom: `${datosFormulario.nombre} ${datosFormulario.apellidos}`,
      client_email: datosFormulario.email,
      comanda_id: comandaId,
      data_comanda: new Date().toLocaleString('es-ES'),
      llista_productes: listaProductosClienteHTML,
      subtotal: formatPrice(comandaData.totals.subtotal),
      descompte: formatPrice(comandaData.totals.descuento),
      envio: formatPrice(comandaData.totals.envio),
      iva: formatPrice(comandaData.totals.iva),
...
${datosFormulario.pais}
    ...
    // ⚠️ REEMPLAÇA AMB LES TEVES CLAUS REALS DEL COMPTE 2 ⚠️
    const serviceIDCliente = 'SERVICE_ID_CUENTA_2';
    const templateIDCliente = 'TEMPLATE_ID_CLIENTE';

    console.log('📧 Enviant email de confirmació al client...');
    await emailjsClientes.send(serviceIDCliente, templateIDCliente, templateParamsCliente);
    console.log('✅ Email de confirmació enviat correctament al client');
