/* app.js - complet i autònom
   ...
   - ENVÍO DE EMAIL AL CLIENTE CON FACTURA
*/
...
/* ---------------------------
   GUARDAR COMANDA I ENVIAR EMAIL (EmailJS) - VERSIÓN MEJORADA
   --------------------------- */
async function guardarComanda(datosFormulario) {
  try {
    ...
    // 3. ENVIAR EMAIL A LA TIENDA amb EmailJS
    const listaProductosTienda = comandaData.productos.map(producto => 
...
${datosFormulario.pais}
      `.trim(),
      llista_productes: listaProductosTienda,
    ...
    const serviceIDTienda = 'xxxx'; // Exemple: service_abc123
    const templateIDTienda = 'xxxx'; // Exemple: template_xyz789

    console.log('📧 Enviant email de comanda a la tienda...');
    await emailjs.send(serviceIDTienda, templateIDTienda, templateParamsTienda);
    console.log('✅ Email enviat correctament a miuartbase@gmail.com');

    // 4. ENVIAR EMAIL DE CONFIRMACIÓN AL CLIENTE
    const listaProductosCliente = comandaData.productos.map(producto => 
      `<tr>
    ...
    const templateParamsCliente = {
      ...
      llista_productes: listaProductosCliente,
      subtotal: formatPrice(comandaData.totals.subtotal),
      descompte: formatPrice(comandaData.totals.descuento),
      envio: formatPrice(comandaData.totals.envio),
      iva: formatPrice(comandaData.totals.iva),
...
${datosFormulario.pais}
    ...
    // ⚠️ REEMPLAÇA AMB LES TEVES CLAUS REALS PER AL CLIENTE ⚠️
    const serviceIDCliente = 'xxxx'; // Pot ser el mateix servei
    const templateIDCliente = 'xxxx'; // Template DIFERENT per al client

    console.log('📧 Enviant email de confirmació al client...');
    await emailjs.send(serviceIDCliente, templateIDCliente, templateParamsCliente);
