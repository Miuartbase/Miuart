...
if (contactForm) {
  contactForm.addEventListener('submit', function(event) {
    ...
    // ✅ VARIABLES MÍNIMAS para contacto
    const templateParams = {
      tipo_mensaje: 'Nou missatge de contacte',
      client_nom: contactName,
      client_email: contactEmail,
      missatge: contactMessage,
      data_comanda: new Date().toLocaleString('es-ES')
      // ⚠️ NO enviamos comanda_id → el template muestra sección CONTACTO
    };
...
if (subtotal >= config.promocionCompraSuperiorA && config.porcentajeDescuento > 0) {
  ...
  console.log('Cupón generado en compra:', cupon.codigo);
    ...
    // 3. Email admin - VERSIÓN DEFINITIVA
...
// ✅ VARIABLES para COMANDA
await emailjs.send('service_z163vmr', 'template_klvyj1r', {
  tipo_mensaje: 'Nova comanda',
  comanda_id: comandaId, // ⚠️ Esta variable activa la sección COMANDA
