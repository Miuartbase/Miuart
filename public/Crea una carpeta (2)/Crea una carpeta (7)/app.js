...
function validarCupon() {
  ...
  // Cupón válido → APLICAR DESCOMPTE
  cuponAplicado = cupon;
  descuentoAplicado = 0; // es recalcularà a aplicarDescuentoCarrito()

  // MARCA EL CUPÓ COM A USAT (AQUÍ ÉS EL MOMENT!)
  cupon.usado = true;
  localStorage.setItem('miuart_cupones', JSON.stringify(cupones));

  // UI: missatge i botons
  ...
  // Aplicar descompte i actualitzar vista
  aplicarDescuentoCarrito();
  renderCart();

  console.log('Cupó aplicat i marcat com a usat:', cupon.codigo);
}
...
function aplicarDescuentoCarrito() {
  ...
  // No cal renderCart() aquí → ja es fa a validarCupon()
}

// Funció per resetear l'estat del cupó (quan es buida el carrito)
...
// ELIMINADA: marcarCuponUtilizado() → ja no cal, es fa dins de validarCupon()
...
async function enviarConfirmacionCliente(comandaData, comandaId) {
  try {
    // === PRODUCTES HTML - AGRUPATS AMB SUBTOTAL ROSA ===
    let productosHTML = `
