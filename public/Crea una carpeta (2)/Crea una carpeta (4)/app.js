...
/* Funciones corregidas: 
   - Sistema de emails corregit: contacte usa template admin
...
/* ---------------------------
   FORMULARI DE CONTACTE (CORREGIT)
   --------------------------- */
if (contactForm) {
  contactForm.addEventListener('submit', function(event) {
    ...
    // ✅ CORRECCIÓ: Utilitzar template d'administrador per a contactes
    const templateParams = {
      comanda_id: 'CONTACTO-' + Date.now(),
      client_nom: contactName,
      client_email: contactEmail,
      client_telefon: 'No especificado',
      client_adreca: 'Formulario de contacto web',
      llista_productes: `
        <tr>
          <td colspan="5">
    ...
    // ✅ CORRECCIÓ: Utilitzar template d'administrador
    emailjs.send('service_z163vmr', 'template_klvyj1r', templateParams)
      .then(() => {
...
/* ---------------------------
   CONFIRMACIÓ DE COMANDA AL CLIENT (CORREGIDA AMB VARIABLES SEGURES)
   --------------------------- */
async function enviarConfirmacionCliente(comandaData, comandaId) {
  try {
    const listaProductos = comandaData.productos.map(producto => 
      `• ${producto.nombre || 'Producto'} (${producto.variante || 'Sin variante'}) - ${producto.cantidad || 0} x ${formatPrice(producto.precio || 0)}`
    ).join('\n');

    // ✅ VARIABLES BÀSIQUES OBLIGATÒRIES (amb valors per defecte)
    const templateParams = {
      order_id: comandaId || 'N/A',
      from_name: comandaData.cliente.nombre || 'Cliente',
      from_email: comandaData.cliente.email || 'N/A',
      message: listaProductos || 'No hay productos en el pedido',
      date: new Date().toLocaleString('es-ES'),
      to_email: comandaData.cliente.email || 'N/A',
      
      // ⭐ VARIABLES DEL TOTAL (assegurar que existeixen)
      subtotal: formatPrice(comandaData.totals?.subtotal || 0),
      descuento: formatPrice(comandaData.totals?.descuento || 0),
      envio: formatPrice(comandaData.totals?.envio || 0),
      iva: formatPrice(comandaData.totals?.iva || 0),
      total: formatPrice(comandaData.totals?.total || 0),
      
      // ⭐ VARIABLES DEL CLIENT (assegurar que existeixen)
      client_name: `${comandaData.cliente.nombre || ''} ${comandaData.cliente.apellidos || ''}`.trim() || 'Cliente',
      client_email: comandaData.cliente.email || 'N/A',
      client_phone: comandaData.cliente.telefono || 'No especificado',
      client_address: `
${comandaData.cliente.direccion || 'Dirección no especificada'}
${comandaData.cliente.infoAdicional || ''}
${comandaData.cliente.codigoPostal || 'CP'} ${comandaData.cliente.ciudad || 'Ciudad'}
${comandaData.cliente.pais || 'España'}
      `.trim()
    };

    // 🆕 VARIABLES DEL CUPÓ (només si existeixen)
    if (comandaData.totals?.subtotal >= config.promocionCompraSuperiorA && config.porcentajeDescuento > 0) {
      const cupones = JSON.parse(localStorage.getItem('miuart_cupones') || '[]');
      const cuponCliente = cupones[cupones.length - 1];
      
      if (cuponCliente && !cuponCliente.usado) {
        templateParams.cupon_codigo = cuponCliente.codigo || 'N/A';
        templateParams.cupon_descuento = cuponCliente.descuento || 0;
        templateParams.cupon_importe_minimo = formatPrice(config.promocionCompraSuperiorA || 0);
      }
    }

    console.log('🔍 DEBUG - Variables enviades al cliente:', templateParams);

    // ✅ CORRECCIÓ: Utilitzar template de client
    templateParams.order_id = templateParams.order_id || "";
    templateParams.from_name = templateParams.from_name || "";
    templateParams.cupon_codigo = templateParams.cupon_codigo || "";
    templateParams.cupon_importe_minimo = templateParams.cupon_importe_minimo || "";
    templateParams.cupon_descuento = templateParams.cupon_descuento || "";
    templateParams.date = new Date().toLocaleDateString();
    templateParams.message = templateParams.message || "";
    templateParams.subtotal = templateParams.subtotal || "";
    templateParams.descuento = templateParams.descuento || "";
    templateParams.envio = templateParams.envio || "";
    templateParams.iva = templateParams.iva || "";
    templateParams.total = templateParams.total || "";
    templateParams.client_name = templateParams.client_name || "";
    templateParams.client_email = templateParams.client_email || "";
    templateParams.client_phone = templateParams.client_phone || "";
    templateParams.client_address = templateParams.client_address || "";

    await emailjs.send('service_z163vmr', 'template_353l90q', templateParams);
    
    console.log('✅ Confirmació enviada al client correctament');
    return true;
    
  } catch (error) {
    console.error('❌ Error enviant confirmació al client:', error);
    // No fem throw - no volem que falli la comanda per l'email
    return false;
  }
}
...
async function guardarComanda(datosFormulario) {
  try {
    ...
    // 3. ENVIAR EMAIL ADMIN amb EmailJS
    const listaProductos = comandaData.productos.map(producto => 
...
${datosFormulario.pais}
    ...
    // Utilitzem el compte principal per a l'admin
    const serviceID = 'service_z163vmr';
    const templateID = 'template_klvyj1r';

    console.log('📧 Enviant email de comanda a admin...');
    await emailjs.send(serviceID, templateID, adminTemplateParams);
    console.log('✅ Email enviat correctament a miuartbase@gmail.com');

    // 4. ENVIAR CONFIRMACIÓ AL CLIENT (CORREGIDA AMB VARIABLES SEGURES)
