...
// NOVA FUNCIÓ: Marcar cupón com a utilitzat
function marcarCuponUtilizado(codigo) {
  ...
  if (cuponIndex !== -1) {
    ...
    localStorage.setItem('miuart_cupones', JSON.stringify(cupones));
    console.log('Cupó marcat com a utilitzat:', codigo);
  }
}
...
async function guardarComanda(datosFormulario) {
  ...
  if (subtotal >= config.promocionCompraSuperiorA && config.porcentajeDescuento > 0) {
    ...
    // POP-UP UNA SOLA VEGADA
    mostrarPopupDescuento(subtotal, codigo);

    console.log('Cupó generat i mostrat:', nuevoCupon);
  }
  ...
  // 3. Email admin
  const listaProductos = comandaData.productos.map(p => 
    `• ${p.nombre} (${p.variante}) - ${p.cantidad} x ${formatPrice(p.precio)} = ${formatPrice(p.precio * p.cantidad)}`
  ).join('\n');
