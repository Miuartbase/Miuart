...
if (contactForm) {
  contactForm.addEventListener('submit', function(event) {
    ...
    // Utilitzem el compte principal per al contacte
    const serviceID = 'service_z163vmr';
    const templateID = 'template_353l90q';
...
/* ---------------------------
   CONFIRMACIÓ DE COMANDA AL CLIENT (NOVA FUNCIÓ)
   --------------------------- */
async function enviarConfirmacionCliente(comandaData, comandaId) {
  try {
    const listaProductos = comandaData.productos.map(producto => 
      `• ${producto.nombre} (${producto.variante}) - ${producto.cantidad} x ${formatPrice(producto.precio)}`
    ).join('\n');

    const templateParams = {
      // Utilitzem les mateixes variables que el formulari de contacte
      from_name: 'Miuart',
      from_email: 'miuartbase@gmail.com',
      message: `CONFIRMACIÓN DE PEDIDO #${comandaId}

Hola ${comandaData.cliente.nombre},
...
Gracias por confiar en Miuart!`,
      date: new Date().toLocaleString('es-ES'),
      to_email: comandaData.cliente.email
    };

    console.log('📧 Enviant confirmació al client...');
    
    // Utilitzem el mateix template que per al contacte
...
${datosFormulario.pais}
    ...
    // 4. NOU: ENVIAR CONFIRMACIÓ AL CLIENT
