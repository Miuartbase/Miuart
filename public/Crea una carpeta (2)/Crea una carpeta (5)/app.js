...
async function enviarConfirmacionCliente(comandaData, comandaId) {
  try {
    // === PRODUCTES HTML ===
    let productosHTML = `<table style="width:100%; border-collapse:collapse; margin:15px 0; font-size:14px;"><thead><tr style="background:#f8f9fa;"><th style="text-align:left; padding:10px; font-weight:bold;">Producto</th><th style="text-align:left; padding:10px; font-weight:bold;">Variante</th><th style="text-align:left; padding:10px; font-weight:bold;">Cant.</th><th style="text-align:left; padding:10px; font-weight:bold;">Precio</th><th style="text-align:left; padding:10px; font-weight:bold;">Subtotal</th></tr></thead><tbody>`;
    for (const p of comandaData.productos) {
      const subtotal = p.precio * p.cantidad;
      productosHTML += `<tr><td style="padding:10px; border-bottom:1px solid #eee;"><strong>${p.nombre}</strong></td><td style="padding:10px; border-bottom:1px solid #eee;">${p.variante}</td><td style="padding:10px; border-bottom:1px solid #eee;">${p.cantidad}</td><td style="padding:10px; border-bottom:1px solid #eee;">${formatPrice(p.precio)}</td><td style="padding:10px; border-bottom:1px solid #eee;">${formatPrice(subtotal)}</td></tr>`;
    }
    productosHTML += `</tbody></table>`;

    // === CUPÓ DES DE localStorage ===
    let cuponHTML = '';
    const cupones = JSON.parse(localStorage.getItem('miuart_cupones') || '[]');
    const ultimo = cupones.length > 0 ? cupones[cupones.length - 1] : null;
    if (ultimo && !ultimo.usado) {
      cuponHTML = `<div style="background:#d4edda; border:1px solid #c3e6cb; border-radius:8px; padding:15px; text-align:center; margin:20px 0;"><h3 style="margin:0 0 10px; color:#155724; font-size:18px;">¡Cupón para tu próxima compra!</h3><p style="margin:5px 0;">Has ganado un <strong>${ultimo.descuento}%</strong> por superar ${formatPrice(config.promocionCompraSuperiorA)}.</p><div style="font-size:24px; font-weight:bold; color:#28a745; background:white; padding:8px 16px; border-radius:6px; display:inline-block; margin:8px 0;">${ultimo.codigo}</div></div>`;
    }
...
async function guardarComanda(datosFormulario) {
  try {
    ...
    // === GENERAR CUPÓ ABANS D'ENVIAR EMAILS ===
    const subtotal = calcularSubtotal();
    if (subtotal >= config.promocionCompraSuperiorA && config.porcentajeDescuento > 0) {
      ...
      mostrarPopupDescuento(subtotal, codigo);
      console.log('Cupó generat i guardat:', nuevoCupon);
    }

    // 2. Guardar a Firestore
    ...
    try {
      ...
      console.log("Comanda no guardada a Firestore, pero email s'enviarà");
    }

    // 3. ENVIAR EMAIL ADMIN
...
${datosFormulario.pais}
    ...
    await emailjs.send('service_z163vmr', 'template_klvyj1r', adminTemplateParams);
    console.log('Email enviat correctament a miuartbase@gmail.com');

    // 4. ENVIAR CONFIRMACIÓ AL CLIENT
